...
public class CodStatView implements Initializable, IStartApp, PropertyChangeListener {
  ...
  private void treeView_eliminaCodstat() {
    // MessageDialog.messageDialog(AlertType.WARNING, "Cancella Cod. Stat. ancora da implementare");
    ...
    Optional<ButtonType> btRet = MessageDialog.messageDialog(AlertType.CONFIRMATION, szMsg, ButtonType.YES);
    ...
    treedata.delete(cds);
    model.firePropertyChange(Consts.EVT_TREECODSTAT_CHANGED, null, cds);
  ...
  private void treeView_esportaCSV() {
    ...
    if ( ! (Utils.isValue(annoComp) && null != treeData)) {
      MessageDialog.messageDialog(AlertType.WARNING, "Mi manca l'anno competenza e la query per formare il nome da esportare");
      return;
    }
    ...
    if ( !bErr)
      MessageDialog.messageDialog(AlertType.INFORMATION, String.format("Scritto CodStat CSV : %s", szCsvFile));
  }
  ...
  void btCercaFileClick(ActionEvent event) {
    ...
    //      s_log.warn(szMsg);
    //      MessageDialog.messageDialog(AlertType, szMsg);
    //    }
    String szMsg = null;
  ...
  void btImportFileClick(ActionEvent event) {
    ...
    if (qtaIdCds > 0) {
      String szMsg = String.format(
          ...
          qtaIdCds);
      butt = MessageDialog.messageDialog(AlertType.CONFIRMATION, szMsg, ButtonType.YES);
    }
    ...
    refreshTreeCodstat();
    // MessageDialog.messageDialog(AlertType.INFORMATION, "??? Funzione Import da implementare");
  }
  ...
  public void propertyChange(PropertyChangeEvent evt) {
    ...
    switch (szEvtId) {
      ...
      case Consts.EVT_NEW_QUERY_RESULT:
        break;

      case Consts.EVT_TOTCODSTAT:
        Platform.runLater(() -> {
      ...
      case Consts.EVT_TREECODSTAT_CHANGED:
